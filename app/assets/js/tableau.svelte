...
async function fetchAsync () {
  ...
  let response = await fetch('http://172.18.0.1/api/entity_peoples');
...
function createTable(data){
  var tableau = document.getElementById('DataTables_Table_0');
  console.log(tableau);
  let api = "none";
  ...
  if(data[0]["@id"].split("/")[2] == "entity_peoples"){
    ...
    str = "<thead class='bg-secondary text-white'>"+
              "<tr>"
                "<th><input type='checkbox' id='select-all'></th>"+
                "<th>Nom</th>"+
                "<th>Prénom</th>"+
  ...
  data.forEach(d => 
      str += "<tr role='row' class='odd'>"+
                  ...
                  "<td>"+d.addDate+"</td>"+
                  "<td>"+
                      "<a href='/manager/people/"+d.id+"'><i class='fas fa-eye'></i></a>"+
                      "<a href='/manager/people/"+d.id+"/edit'><i class='far fa-edit'></i></a>"+
                  "</td>"+
                "</tr>"
  );
  tbody.innerHTML += str;
